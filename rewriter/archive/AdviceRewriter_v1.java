...
public class AdviceRewriter {
	...
	public AdviceRewriter () throws IOException{
		...
		// write the advice file
		writeAdviceFile();	

		ArrayList<String> c = classOfFV("X");
		for(int l = 0; l < c.size(); l++){
			System.out.println(c.get(l));
		}

		/*ArrayList<JSONObject> p = predicateClass("T");
		for(int l = 0; l < p.size(); l++){
			System.out.println(p.get(l));
		}*/

		String q = predicateClassConjunct("X");
		System.out.println(q);
		System.out.println(intermediateQueryResponseFormat("Y"));
		

		ArrayList<String> t = triggersOfPredicateClass("Y");
	...
	private void initializeData() throws IOException{
		/* read the json file */
		String jsonContent = readFile("LS_MEM_LEAK.json");
		/* parse the json file*/
	...
	private void writeConfigXML() throws IOException{
		String configTemplate = readFile("config_template");
		String rewrittenConfig;

		String define_advice_points = "";

		for (int i = 0; i < advicePoints.size(); i++) {
			define_advice_points += "<advice>\n\t\t<point>" + advicePoints.get(i) + "</point>\n\t\t<class>org.openmrs.module.example6.advice.PatientServiceAdvice</class>\n\t</advice>\n\n\t";
		}
		rewrittenConfig = configTemplate.replace("<ADVICES>", define_advice_points);
		writeFile("config.xml", rewrittenConfig);
	}


	private void writeAdviceFile() throws IOException {
		String adviceTemplate = readFile("advice_template");
		String rewrittenAdvice;


		/* define the predicates */
		rewrittenAdvice = adviceTemplate.replace("<DEF_PREDS>", definePredicates());		

		/* assert the horn clauses and list extra condtion variables*/
		rewrittenAdvice = rewrittenAdvice.replace("<ASSERT_CLAUSES>", assertClauses());		

		rewrittenAdvice = rewrittenAdvice.replace("<TRIGGER_COUNT>", "" + triggerCount);
		...
		writeFile("PatientServiceAdvice.java", rewrittenAdvice);				
	}
	...
	private String definePredicates() {
		...
		for(int i = 0; i < headLiteralName.length; i++){
			if (headLiteralSymbolType[i].equals("user_defined")){
				define_predicates += "engine.command(\"dynamic " + headLiteralName[i] + "/" + headLiteralArgName[i].length +" as incremental\"); \n\t\t\t";
			}
			
			for (int k = 0; k < bodyLiteralName[i].length; k++){
				if (bodyLiteralSymbolType[i][k].equals("user_defined")){
					define_predicates += "engine.command(\"dynamic " + bodyLiteralName[i][k] + "/" + bodyLiteralArgName[i][k].length +" as incremental\"); \n\t\t\t";
				}				
			}
		}
	...
	private String assertClauses() {
		...
		for(int i = 0; i < headLiteralName.length; i++){
			assert_clauses += "engine.command(\"assert(("; 
				...
				for (int k = 0; k < bodyLiteralName[i].length; k++){
					...
					if (bodyLiteralName[i][k].equals("funccall")){ // if the body literal is an event or trigger
						...
						if (isLogEvent(funccallMethodName)){ //RelevantCall-8
							check_calls += "if (method.getName().equals(\"" + funccallMethodName + "\")){\n\t\t\t\t" + "engine.command(\"assert(funccall(\"  + preciseTimestamp(precision, ts) + \", \"  + method.getName() + \", \" + currentUser <POTENTIAL_ARGS> + \"))\"); \n\t\t\t}\n\t\t\t";
						}
							...
							if (corrPredicateClass.size() == 1) { //RelevantCall-2
								...
								if (classFV.size() == numberOfFV) { // RelevantCall-3 and RelevantCall-4
									check_calls += "if (method.getName().equals(\"" + funccallMethodName + "\")){\n\t\t\t\t" + "if (!w.contains(\"" + funccallMethodName +"\")){\n\t\t\t\t\t" + "engine.command(\"assert(funccall(\"  + preciseTimestamp(precision, ts) + \", \"  + method.getName() + \", \" + currentUser <POTENTIAL_ARGS> + \"))\"); \n\t\t\t\t\tGG1 = \"findall(TM, ("+ corrPredicateClassConjunct +",buildTermModel("+ corrIQRF +",TM)), L), ipObjectSpec('ArrayOfObject',L,LM)\";\n\t\t\t\t\tsolutions = (Object[])engine.deterministicGoal(GG1,\"[LM]\")[0];\n\t\t\t\t\tif (solutions.length > 0) {\n\t\t\t\t\t\tw.add(\"" + funccallMethodName + "\");\n\t\t\t\t\t}\n\t\t\t\t\telse{\n\t\t\t\t\t\tengine.command(\"retract(funccall(\"  + preciseTimestamp(precision, ts) + \", \"  + method.getName() + \", \" + currentUser  + \"))\");\n\t\t\t\t\t}\n\t\t\t\t}\n\t\t\t}\n\t\t\t";
								}
								else{
